...
#ifdef __KERNEL__
static struct timer_list current_timer;
#endif
...
void ip_noise_delayer_check_pq(ip_noise_delayer_t * delayer)
{
    ...
    if (msg != NULL)
    {
        ...
        init_timer(&current_timer);
        
        current_timer.expires = jiffies + (HZ * num_millisecs);
        current_timer.data = (unsigned long)delayer;
        current_timer.function = (void (*) (unsigned long))ip_noise_delayer_check_pq;
        add_timer(&current_timer);
    }
#endif
